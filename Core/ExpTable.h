...
//	CScreen::Add(StatusWindow, 0, " ==================== Server Statistics====================");

	CScreen::Add(StatusWindow, 0, " %s", getUptime().c_str());
	CScreen::Add(StatusWindow, 0, " %s", getSysUpTime().c_str());

	
	CScreen::Add(StatusWindow, 0, " Online Players: %d", InterlockedExchangeAdd(&OnlinePlayers, 0));
	CScreen::Add(StatusWindow, 0, " Fake Shops: %d", EFakePlayers);
	CScreen::Add(StatusWindow, 0, " Active Channels: %d", ChannelActivated);
	CScreen::Add(StatusWindow, 0, " Blocked Players: %d", PlayerBlockCheck.size());

//	CScreen::Add(StatusWindow, 0, " Top Of MMOs Votes: %d", VoteList.size());

	CScreen::Add(StatusWindow, 0, " =====================Registrations Statistics=====================");
	CScreen::Add(StatusWindow, 0, " Lottery: %d", Lottery.size());
	CScreen::Add(StatusWindow, 0, " Lotto: %d", XLottery.size());
	CScreen::Add(StatusWindow, 0, " Boss EXP: %d", BossEXPRegistration.size());
	CScreen::Add(StatusWindow, 0, " F10 Company: %d", F10Registration.size());
	CScreen::Add(StatusWindow, 0, " Mautareta: %d", MautRegistration.size());
	CScreen::Add(StatusWindow, 0, " Raid: %d", RaidSystem.size());
	CScreen::Add(StatusWindow, 0, " Battlefield: %d", BattlefieldRegistration.size());
	CScreen::Add(StatusWindow, 0, " Triangular Battle: %d", TBRegistration.size());
	CScreen::Add(StatusWindow, 0, " Capture The Flag: %d", CaptureRegistration.size());
	CScreen::Add(StatusWindow, 0, " Last Man Standing: %d", LastManRegistration.size());
...
int __cdecl AttackMsg(void* Player, const char Type, const char* Format, int id1, int id2, int Damage, int EBDamage, int GetType) {
	...
	/*if (Attacker.IsOnline()) {
		...
		while (Around)
		{
			...
			if (Attacker.IsValid() && Object.IsValid() && Object.GetOffset() != Attacker.GetOffset() && Object.GetType() == 0 && (!(Attacker.GetType() == 1 && Attacker.GetDef() == 54123 && *(DWORD *)((int)Player + 232) == (int)Object.GetOffset()) || Attacker.GetType() == 0) && Object.GetBuffValue(BuffNames::TargetAttack) == id2)
				CPlayer::Write(Object.GetOffset(), 0xFE, "dddddbd", 189, Object.GetID(), id2, 0, 1, GetType, 0);
...
void ServerStartedMessage() {
	DrawMainMenu("Scania Developments");
