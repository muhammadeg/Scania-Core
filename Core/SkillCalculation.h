...
double getSoustraction(std::string str,std::string type){
	...
	double plus=0;
	std::string t = type+"_%lf";
	std::string t1 = type+"_ %lf";
	std::string t2 = type+" _%lf";
	std::string t3 = type+" _ %lf";
	if((sscanf(str.c_str(), t.c_str(), &plus) == 1) || (sscanf(str.c_str(),t1.c_str(), &plus) == 1) || (sscanf(str.c_str(), t2.c_str(), &plus) == 1) || (sscanf(str.c_str(), t3.c_str(), &plus) == 1))
		return plus;
	else
		return 0;
	...
	if (text.find(parameter) != std::string::npos || !text.compare(parameter)) {
		...
		if (Addition) {
			...
			if (before.size() > 0) {
				...
				else if (!before.compare("_"))  *dmg -= (value + plus);
...
//		}
//
//		if (TargetType == 0 && sDmg.PVPDamage > 0)
//			dmg = (dmg * sDmg.PVPDamage) / 100;
//
//		if (TargetType == 1 && sDmg.PVEDamage > 0)
//			dmg = (dmg * sDmg.PVEDamage) / 100;
//
//		if (TargetType == 0 && sDmg.Reduce > 0)
//			dmg = (dmg * (sDmg.Reduce)) / 100;
//
