...
int BofConfigRead = 0, dgConfigRead = 0, insanityRead = 0, qigongRead = 0, ImpConfigRead = 0, NailKill = 0, Bof2ConfigRead = 0, Imp2ConfigRead = 0;
...
int JewelIndex = 0;
int HonorNotices = 0, ImperialNotices = 0, BofNotices = 0;
int Gun = 0;
...
int PPSys = 0, PPSysB = 0, PPEXP = 0, CJBEXP = 0, CJBSYS = 0, CJBSYSB = 0, CJBEXPActive = 0, CJBRange = 0, PPActive = 0, My_PPActive = 0, My_PPRange = 0;
int itemIndex = 0;
...
void ReadConfig(bool command)
{
	...
	My_PPRange = GetPrivateProfileIntA("Bonus", "Range", 300, "./Configs/PerfectParty.txt");
	...
	HSRange = GetPrivateProfileIntA("HalfSwing", "Range", 1, "./Skills/Knight.txt");
	...
	if (!command || (command && modifiedFiles.count("./Configs/Reborn.txt"))) {
		...
		if (fileo != NULL)
		{
			...
			while (fgets(line, sizeof line, fileo) != NULL)
			{
				...
				int Map = 0, X = 0, Y = 0;
				char tag[10];
				if (sscanf(line, "(Reborn (Num %d)(NameTag '%[a-z | A-Z | 0-9/<>|.,~*;`:!^+%&=?_-£#$€]')(NamePad %d)(MinLvl %d)(ResetLvl %d)(RewardID %d)(SBKey %d)(SBMsg %d)(Teleport %d %d %d))", &ID, &tag, &NamePad, &MinLvl, &ResetLevel, &RewardID, &sbKey, &sbMsg, &Map, &X, &Y) >= 6)
				{
					Reborn rb = Reborn();
					rb.NamePad = NamePad;
					rb.MinLvl = MinLvl;
