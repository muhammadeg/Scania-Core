...
public:
	...
	std::string ostream2string(std::ostream& content);
	char *_Compile(char* Destination, const char *Format, ...);
	char *_Compile(char* Destination, std::string Format, va_list vArgs);
	...
	__int64 Per_calculation_int64(int add, __int64 value);
